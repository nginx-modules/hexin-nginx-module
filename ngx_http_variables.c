 ...
 */
/*
 *  将服务器时间 通过变量timemills 给暴露出来    是一个毫秒级别的长整形数
 */
...
static ngx_int_t ngx_http_variable_binary_remote_addr(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
//add by yinhongzhan
static ngx_int_t ngx_http_timemillis(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_http_variable_t  ngx_http_core_variables[] = {
  ...
  //add by yinhongzhan
	{ ngx_string("timemills"), NULL, ngx_http_timemillis, 0, 0, 0 },
...
//add by yinhongzhan
static ngx_int_t ngx_http_timemillis(ngx_http_request_t *r,  ngx_http_variable_value_t *v, uintptr_t data)
{
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "进入ngx_http_timemillis方法...");
	struct timeb tp;
	ftime(&tp);
	long l  = tp.time * 1000 + tp.millitm;
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "当前时间timemillis数－long－为 : [%l]...",  l);
	...
	ngx_sprintf(v->data, "%l", l);
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "当前时间timemillis数-varchar-为 : [%s]...",  v->data);
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "退出ngx_http_timemillis方法...");
	return NGX_OK;
}
